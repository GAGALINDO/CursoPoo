...
/*agrregandole lo que le hace falta*/ /* se usa el where para evitar actualizar todda la tabla*/
...
/*where va al final y se pone el ID Primario para identifica donde se modificara*/

/*borra donde el id sea ese*/
...
/* seleccioname todos los datos de la tabla 1 y todos los datos de la tabla 2 cuando el campo coincida de ambas*/
...
/*match une las coincidencias ( aqui van las coincidencias em dpde buscara)         */
SELECT * FROM movieseries
	WHERE MATCH(title, author, actors, genres)
	/*en againstr va la palabra que se busca*/
	AGAINST('stallone' IN BOOLEAN MODE);

/*Seleccionando un par de campos*/
