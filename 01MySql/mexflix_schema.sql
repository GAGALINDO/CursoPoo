...
CREATE TABLE movieseries(
	...
	title VARCHAR(80) NOT NULL,/*VARCHAR es para adptarse al tañamo del campo y no puede ir vacio not null*/
	plot TEXT,
	author VARCHAR(100) DEFAULT 'Pending',/*Default espara poner un valor por defecto*/
	actors VARCHAR(100) DEFAULT 'Pending',
	country VARCHAR(30) DEFAULT 'Unknown',
	premiere YEAR(4) NOT NULL,
	poster VARCHAR(150) DEFAULT 'no-poster.jpg',/*se usa varchar por que se usa un URL*/
	trailer VARCHAR(150) DEFAULT 'no-trailer.jpg',
	rating DECIMAL(2,1),/*numero de dos campos y uno sera decimal*/
	genres VARCHAR(50) NOT NULL,
	status INTEGER UNSIGNED NOT NULL,/*hace referencia a una tabla*/
	category ENUM('Movie', 'Serie') NOT NULL,
	FULLTEXT KEY search(title, author, actors, genres),/*busqyeda fulltext para buscarlo que esta en ()*/
	FOREIGN KEY (status) REFERENCES status(status_id) /*llave foranea,la tabla de la llave forania debe estar antes*/
		ON DELETE RESTRICT ON UPDATE CASCADE/*que va a pasar cuando se elimine y se inserte*/
		/*cascade cuando en status se haga un cambio en automatico se vera*/
);

/*Tabla de Usuarios*/
CREATE TABLE USERS(
	user VARCHAR(15) PRIMARY KEY,
	email VARCHAR(80) UNIQUE NOT NULL,/*unique hace referencia a que no se puedan volver a insertar*/
	name VARCHAR(100) NOT NULL,
	birthday DATE NOT NULL,
	pass CHAR(32) NOT NULL,/*de 32 por que se encripta con md5 por que se hacne de 32*/
	role ENUM('Admin', 'User') NOT NULL/*TENUM lista enumerada*/
);
/*insertando en catalogo de dtos*/
...
INSERT INTO USERS (user, email, name, birthday, pass, role) VALUES
	('@jonmircha', 'jonmircha@bextlan.com', 'Jonathan MirCha', '1984-05-23', MD5('chafo'), 'Admin'),/*md5 para encriptar*/
	('@user', 'usuario@bextlan.com', 'Usuario Mortal', '2000-12-19', MD5('chimichangas'), 'User');

/*Insertar datos en las tablas, se usa OMBD API*/
/*http://omdbapi.com es una API abierta, buscmos y lo sacamos como JSON*/
