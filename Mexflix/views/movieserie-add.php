<?php 
//metodo para agregar y que se almacene a la BDD
if( $_POST['r'] == 'movieserie-add' && $_SESSION['role'] == 'Admin' && !isset($_POST['crud']) ) {

	//gracias a autolog puedo acceder a cualqier controlarod a continuación

	$status_controller = new StatusController();
	$status = $status_controller->get();
	$status_select = '';//va a tomar los opcion de mi selet 

	//mientras que n sea al numero menor al numero de elementos que contiene la base de datos 
	for ($n=0; $n < count($status); $n++) { 

		$status_select .= '<option value="' . $status[$n]['status_id'] . '">' . $status[$n]['status'] . '</option>';
	}

	//CAMPOS PARA QUE SE RELLENE LA BDD PRINTF 
	printf('
