<?php 
//manda a allamar movieseriescontroller anterior mente hecho
...
if( $_POST['r'] == 'movieserie-edit' && $_SESSION['role'] == 'Admin' && !isset($_POST['crud']) ) {
	...
	if( empty($ms) ) {
		...
		for ($n=0; $n < count($status); $n++) { 
			//comparamos si el valor de movie o serie 
			//
			$selected = ($ms[0]['status'] == $status[$n]['status']) ? 'selected' : '';
		//concatenamos la variable selected
			$status_select .= '<option value="' . $status[$n]['status_id'] . '"' . $selected . '>' . $status[$n]['status'] . '</option>';
		}

		//el formulario sera similiar en todos los caoss ""VERIFICAR DIFERENCIAS"
		$template_ms = '
	...
	//establece el metodo ser que pasa save_ms
	$ms = $ms_controller->set($save_ms);
	//redegirir la pagina amovie series
	$template = '
		<div class="container">
			<p class="item  edit">MovieSerie <b>%s</b> edición exitoso</p>
		</div>
		<script>
			window.onload = function () {
				reloadPage("movieseries")
